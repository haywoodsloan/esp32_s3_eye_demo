...
- **Detector confidence floor** — a per-orient floor selected by
  `min_detect_score_for(try_orient)`:
  - `MIN_DETECT_SCORE_ROT0 = 0.35` for the default upright orient.
    Loose enough that lens reflections / refraction from glasses
    don't push real detections under the floor.
  - `MIN_DETECT_SCORE_ROTATED = 0.50` for the three non-default
    orients. Cost of letting a marginal wrong-orient through is high
    (the cycle locks there, banner now rotates with the lock); cost
    of demanding stronger evidence is one or two extra cycle rounds.
  The `keypoints_look_upright` gate below provides the second line of
  defence against the upside-down / textured-blob false-positive
  class.
...
| `MIN_DETECT_SCORE_ROT0` | 0.35 | Model-confidence floor when the detector ran with no pre-rotation. Loose enough to admit glasses-wearer detections (lens reflections / refraction reliably knock ~0.05 off the model's score). |
| `MIN_DETECT_SCORE_ROTATED` | 0.50 | Stricter floor for the three non-default orients. Cost of accepting a weak rotated detection is high (cycle locks there, banner reads upside-down or sideways); cost of demanding more evidence is just an extra cycle round. |
