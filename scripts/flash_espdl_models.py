...
# Source models bundled with each component. We use the ESPDet PICO
# 224x224 single-stage detector (~496 KB) and the MobileFaceNet
# embedder (~1.3 MB). These selections must match the Kconfig defaults
# in sdkconfig.defaults; change both together if you swap models.
DETECT_SOURCES = [
...
# Partition offsets MUST match partitions.csv. human_face_det starts
# right after the 4 MB factory app; human_face_feat follows after the
# 512 KB face-det partition.
